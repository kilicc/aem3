...
-- 2. service_id'yi nullable yap (Ofis işleri için hizmet gerekmez)
ALTER TABLE work_orders 
  ALTER COLUMN service_id DROP NOT NULL;

-- 3. İş tipi ekle (müşteri işi mi, ofis işi mi)
ALTER TABLE work_orders 
  ADD COLUMN IF NOT EXISTS work_type TEXT CHECK (work_type IN ('customer', 'office')) DEFAULT 'customer';

-- 4. Ofis işleri için yapılacak iş açıklaması
ALTER TABLE work_orders 
...
-- 6. Constraint: Eğer work_type = 'customer' ise customer_id ve service_id zorunlu olmalı
-- Bu kontrolü uygulama katmanında yapacağız, veritabanı constraint'i çok karmaşık olur

-- 7. RLS politikaları güncelle (gerekirse)
